...
import ControlesToque from './ControlesToque';
...
export default function Hud({ onRestart, onAbort }: HudProps) {
  ...
  const [isToque] = useState(telaDeToque);
  ...
  useFrame(() => {
    ...
    if (distanciaRef.current) {
      ...
      distanciaRef.current.innerText = `${isToque ? 'DIST' : 'DISTÂNCIA'}: ${distFormatada} M`;
    }

    if (velocidadeRef.current) {
      const velFormatada = Math.floor(ESTADO_JOGO.velocidadeAtual * 10).toString().padStart(3, '0');
      velocidadeRef.current.innerText = `${isToque ? 'VEL' : 'VELOCIDADE'}: ${velFormatada} KM/H`;
    }
  ...
  return (
    <Html fullscreen>
      ...
      {isToque ? (
        <div className="absolute top-4 right-4 flex flex-col items-end gap-1 pointer-events-none text-[#03e43f] font-mono text-xs font-bold tracking-widest drop-shadow-[0_0_8px_rgba(3,228,63,0.8)]">
          <div ref={distanciaRef}>DIST: 00000 M</div>
          <div ref={pontosRef} className="text-[#f1950a] drop-shadow-[0_0_8px_rgba(241,149,10,0.8)]">PONTOS: 00000</div>
          <div ref={velocidadeRef}>VEL: 000 KM/H</div>
        </div>
      ) : (
      ...
      {isToque && !isGameOver && <ControlesToque />}

      {isGameOver && (
        <div className="absolute inset-0 bg-black/90 backdrop-blur-md flex flex-col items-center justify-center overflow-y-auto px-4 py-6 text-[#03e43f] font-mono pointer-events-auto transition-opacity duration-1000 z-50">
          <h1 className="text-4xl sm:text-5xl md:text-7xl text-center font-bold mb-4 text-red-500 drop-shadow-[0_0_15px_rgba(239,68,68,0.8)] tracking-widest">
            FALHA CRÍTICA
          </h1>
          
          <div className="text-center mb-6 md:mb-10 bg-[#050505] border border-[#03e43f] p-5 md:p-8 rounded-sm shadow-[0_0_20px_rgba(3,228,63,0.15)] flex flex-col gap-3 w-full max-w-md">
             <p className="text-base md:text-xl">DISTÂNCIA: <span className="text-white font-bold">{Math.floor(ESTADO_JOGO.distancia)} M</span></p>
             <p className="text-base md:text-xl">PONTOS DE COMBATE: <span className="text-[#f1950a] font-bold">{ESTADO_JOGO.pontos}</span></p>
          ...
          <div className="flex gap-4 md:gap-6 flex-row">
            <button 
              onClick={onRestart} 
              className="px-6 md:px-10 py-3 bg-[#03e43f]/10 border-2 border-[#03e43f] text-[#03e43f] font-bold uppercase tracking-widest hover:bg-[#03e43f] hover:text-[#050505] transition-all"
            >
              Reiniciar
            </button>
            <button 
              onClick={onAbort} 
              className="px-6 md:px-10 py-3 bg-red-500/10 border-2 border-red-500 text-red-500 font-bold uppercase tracking-widest hover:bg-red-500 hover:text-[#050505] transition-all"
            >
              Sair
            </button>
          </div>
